...
/**
 * <B>TheStringTokenizer</B> is a replacement for java.util.TheStringTokenizer (because it sucks!). Please note that this class uses the support.Tokenizer class. If that class is changed then this will break!
 * <p/>
 * <p/>
 * 
 * @author Nazmul Idris
...
public class TheStringTokenizer
{
	 ...
	 * <p>
	 * For example: <tt>t.set( "LOR;ispq.com;4900" , ";" );</tt> will yield {"LOR","ispq.com"} when getTokens() is called. Whereas StringTokenizer would return {"LOR", "ispq.com","4900"}
	 * <p/>
	...
	public class Tokenizer
	{
		 ...
		 * 
		 * @param input
		 *            message buffer that contains the untokenized data received from the socket
		 * @param delim
		 *            delimiter string used to delimit the tokens in the message buffer.
		 * 
		 * @return returns a reference to this object (just as a convenience) to invoke other methods on it.
		 */
		...
		/**
		 * This is a self test method that puts the Tokenizer class through its paces to determine that it works as advertised.
		 * 
		 * @param args
		 */
		/*
		 * public static void main( String[] args ){ Tokenizer t = new Tokenizer();
		 * 
		 * String noTokenInput = "20,LOR;ispq.com;4900"; String input = "20,LOR;ispq.com;4900<T>30,LOR;ispq.com;4900<T>40,LOR;ispq.com;490"; String inputWithNoRemainder = "20,LOR;ispq.com;4900<T>30,LOR;ispq.com;4900<T>"; String delim = "<T>";
		 * 
		 * 
		 * //this is for input with tokens System.out.println( "***valid input with tokens***" ); System.out.println( "input:"+input ); t.set( input , delim ); System.out.println( "hasTokens():"+t.hasTokens() );
		 * 
		 * String[] strRay = t.getTokens(); for( int i=0; i<strRay.length; i++) { System.out.println( "token:"+strRay[i] ); } System.out.println( "t.hasRemainder():"+t.hasRemainder() ); System.out.println( "t.getReminderOfInput():"+t.getRemainderOfInput() );
		 * 
		 * 
		 * //this is for input with tokens (but no remainder) System.out.println( "\n\n***valid input with tokens and no remainder***" ); System.out.println( "input:"+inputWithNoRemainder ); t.set( inputWithNoRemainder , delim ); System.out.println( "hasTokens():"+t.hasTokens() );
		 * 
		 * strRay = t.getTokens(); for( int i=0; i<strRay.length; i++) { System.out.println( "token:"+strRay[i] ); } System.out.println( "t.hasRemainder():"+t.hasRemainder() ); System.out.println( "t.getReminderOfInput():"+t.getRemainderOfInput() );
		 * 
		 * 
		 * //this is for input without tokens System.out.println( "\n\n***invalid input with no tokens***" ); t.set( noTokenInput , delim ); System.out.println( "input:"+noTokenInput ); System.out.println( "hasTokens:"+t.hasTokens() ); if( t.getTokens() == null ) { System.out.println( "input does not contain any tokens" ); } System.out.println( "t.hasRemainder():"+t.hasRemainder() ); System.out.println( "t.getReminderOfInput():"+t.getRemainderOfInput() );
		 * 
		 * 
		 * }
		 */
