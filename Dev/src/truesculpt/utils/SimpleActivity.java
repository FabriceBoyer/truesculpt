  ...
  try {
    ...
    switch (resultCode) {
      case Activity.RESULT_CANCELED:
        Log.i(Global.TAG3, this.getClass().getSimpleName() +
                           " - Activity returned result [CANCEL]:" +
                           data.toString());
...
/**
 * simply runs a fadeout anim on the {@link #rootView}, and then call the super class's implementation of
 * the back button press eventhandler.
 */
public void runFadeOutAnimAndFinish(final int i, final KeyEvent event) {

  if (rootView == null) throw new IllegalArgumentException("rootView can not be null!");
  ...
  animation.setAnimationListener(new Animation.AnimationListener() {
	...
	public void onAnimationEnd(Animation animation) {
      ...
      if (i == 0 && event == null) finish();
      else SimpleActivity.super.onKeyDown(i, event);
    }
    @Override
...
/**
 * simply calls the {@link #runFadeOutAnimAndFinish(int, KeyEvent)} with some params that will cause it
 * not to call the super class's implpementation of KEYCODE_BACK pressed
 */
