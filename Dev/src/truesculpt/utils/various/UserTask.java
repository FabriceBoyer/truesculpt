 ...
 * 
 * <p>
 * A user task is defined by a computation that runs on a background thread and whose result is published on the UI thread. A user task is defined by 3 generic types, called <code>Params</code>, <code>Progress</code> and <code>Result</code>, and 4 steps, called <code>begin</code>, <code>doInBackground</code>, <code>processProgress<code> and <code>end</code> .
 * </p>
 * 
 * <h2>Usage</h2>
 * <p>
 * UserTask must be subclassed to be used. The subclass will override at least one method ({@link #doInBackground(Object[])}), and most often will override a second one ({@link #onPostExecute(Object)}.)
 * </p>
 * 
 * <p>
 ...
 * <li>{@link #doInBackground(Object[])}, invoked on the background thread immediately after {@link # onPreExecute ()} finishes executing. This step is used to perform background computation that can take a long time. The parameters of the user task are passed to this step. The result of the computation must be returned by this step and will be passed back to the last step. This step can also use {@link #publishProgress(Object[])} to publish one or more units of progress. These values are published on the UI thread, in the {@link #onProgressUpdate(Object[])} step.</li>
 * <li>{@link # onProgressUpdate (Object[])}, invoked on the UI thread after a call to {@link #publishProgress(Object[])}. The timing of the execution is undefined. This method is used to display any form of progress in the user interface while the background computation is still executing. For instance, it can be used to animate a progress bar or show logs in a text field.</li>
 * <li>{@link # onPostExecute (Object)}, invoked on the UI thread after the background computation finishes. The result of the background computation is passed to this step as a parameter.</li>
 * </ol>
 * 
 ...
 * <li>Do not call {@link # onPreExecute ()}, {@link # onPostExecute (Object)}, {@link #doInBackground(Object[])}, {@link # onProgressUpdate (Object[])} manually.</li>
...
public abstract class UserTask<Params, Progress, Result>
{
	 ...
	 *             If {@link #getStatus()} returns either {@link UserTask.Status#RUNNING} or {@link UserTask.Status#FINISHED}.
	 */
